...
async function WAStart() {
  ...
  const client = WAConnect({
    ...
    auth: state,
    connectTimeoutMs: 3 * 60 * 1000 // 3 minutes (180 seconds)
  });
  ...
  for (const key in settings) {
    if (settings[key] === true && key !== 'sessionPath' && key !== 'browserPlatform' && key !== 'browserName' && key !== 'noAntiRestart' && key !== 'ignoreError') {
      activeFeatures++;
    }
  }

  // Count inactive features
  let inactiveFeatures = Object.keys(settings).length - activeFeatures - 6; // Subtract sessionPath, browser settings, noAntiRestart, ignoreError, and 5 additional settings

  console.log(`\nFitur yang diaktifkan: ${activeFeatures}`);
  console.log(`Fitur yang dinonaktifkan: ${inactiveFeatures}`);
